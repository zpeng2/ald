...
class Maxpass(ald.Callback):
    def __init__(self, runner):
        super().__init__(runner)

    def __call__(self, i, cfg):
        if self.runner.iscomputing(i):
            print("maximum passx= {:d}".format(gpuarray.max(cfg.passx).get()))
...
def simulate(lH=1, alpha=1.2):
    ...
    compiler.log2file("debug.cu")
    compiler.compile()

    simulator = ald.RTPSimulator(cfg, compiler)

    # setup callbacks.
    file = "debug_lH{:.3f}alpha{:.3f}.h5".format(U0, alpha)
    ...
    configsaver = ald.ConfigSaver(
        ald.RangedRunner(start=14_000_000, stop=16_000_000, freq=10000),
        file,
        variables=["x", "y", "theta"],
    ...
    debug = Maxpass(ald.RangedRunner(start=0, stop=16_000_000, freq=1000))
    # y = ald.MeanVariance(runner, "y", unwrap=True)
    callbacks = [eta, configsaver, debug]
